...
## Database is PostgreSQL-only (resolved — Neon in production)

The app now uses **PostgreSQL exclusively** (including local dev, which previously used SQLite).
The single `prisma/schema.prisma` has `provider = "postgresql"`, so the Prisma Client generated by
`postinstall`/`vercel-build` is always the Postgres client — there is no longer a schema mismatch
risk between dev and deploy.

**What changed:** the separate `prisma/schema.postgres.prisma` mirror was removed; the default
schema is now Postgres. `DATABASE_URL` must point at a Postgres instance (see `.env.example`). The
Neon database used in production was provisioned and seeded (34 phonemes / 11 drills / 42
drill_phoneme), and the live app served `/api/drills`, `/api/phonemes`, and `/api/mastery` with 200s
against it. `prisma/seed.js` is unchanged and works against the Postgres `DATABASE_URL`.
